...
def init_schema():
  ...
  con.execute(
    ...
    'devel_unused_image_revisions_view(image_title, revision_timestamp, revision_size) AS '
  ...
  con.execute(
    'CREATE VIEW IF NOT EXISTS '
    'devel_used_image_revisions_view(image_title, revision_timestamp, revision_size) AS '
    'SELECT images.title, revisions.timestamp, revisions.size FROM images '
    'INNER JOIN revisions ON images.id = revisions.image_id '
  ...
  con.execute(
    'CREATE VIEW IF NOT EXISTS '
    'devel_older_used_image_revisions_view(image_title, revision_timestamp, revision_size) AS '
    'SELECT image_title, revision_timestamp, revision_size FROM devel_used_image_revisions_view AS t1 '
    'WHERE revision_timestamp NOT IN '
      '(SELECT MAX(revision_timestamp) FROM devel_used_image_revisions_view AS t2 '
...
def search(usage: QueryParams.Usage,
  ...
  match usage:
    case QueryParams.Usage.unused:
      ...
      query_str = query_base.format(view = 'devel_unused_image_revisions_view') + query_end
    case QueryParams.Usage.used:
      if include_newest:
        #Will look for all revisions in used images
        query_str = query_base.format(view = 'devel_used_image_revisions_view') + query_end
      else:
        #Will look for older revisions in used images
        query_str = query_base.format(view = 'devel_older_used_image_revisions_view') + query_end
    case QueryParams.Usage.any:
      if include_newest:
        used_image_view = 'devel_used_image_revisions_view'
      else:
        used_image_view = 'devel_older_used_image_revisions_view'

      #Will look for all revisions in unused images and either all or older revisions in used images
      query_str = query_base.format(view = 'devel_unused_image_revisions_view') + 'UNION ALL ' +\
