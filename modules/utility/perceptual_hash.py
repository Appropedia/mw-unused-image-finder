...
def _input_thread(pipe: io.BufferedReader, queue: queue.Queue):
  while True:
...
def _resize_image_if_needed(stream: Iterator[bytes]) -> tuple[Status, int, bytes | None]:
  ...
  stdout_thread = threading.Thread(target = _input_thread,
